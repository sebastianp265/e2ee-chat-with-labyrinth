            ...
            on("task", {
                async runSQLs({filenames}) {
                    ...
                    for (const filename of filenames) {
                        const sql = readFileSync(`cypress/database/${filename}`, "utf-8");
                ...
                connectWebSocket({connectionName, cookie}) {
                    return new Promise((resolve, reject) => {
                        const ws = new WebSocket("http://localhost:8080/api/ws", {
                            headers: {
                                cookie,
                            },
                        })
                        ...
                        ws.on("message", (message) => {
                            try {
                                ...
                                if (messagesPerConnection[connectionName]) {
                                    messagesPerConnection[connectionName].push(decodedMessage)
                                } else {
                                    messagesPerConnection[connectionName] = [decodedMessage]
                                }
                            } catch (err) {
                                reject(err)
                            }
                ...
                async getSessionCookieOfUser({username}) {
                    try {
                        ...
                        const cookie = cookies.filter((c) => c.includes("SESSION="));
                        if (cookie.length != 1) {
