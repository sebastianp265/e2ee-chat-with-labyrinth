import {NewChatMessageToSendPayload, NewChatThreadToSendPayload} from "@/pages/messages/hooks/useChatWebSocket.ts";
...
describe("Creating threads", () => {
    beforeEach(() => {
        cy.intercept('POST', '/api/chat-service/threads/*/messages').as('store-message');
    })

    function createThread(withUsers: UserPool[], userMessagePairs: [UserPool, string][]) {
        cy.get("button")
            .click()

        cy.get("div[cmdk-list]").should('be.hidden')
        cy.get("input[cmdk-input]").click()
        cy.get("div[cmdk-list]")
            .within(() => {
                for (const userToAdd of withUsers) {
                    cy.get(`div[data-value="${userPoolToDetails[userToAdd].visibleName}"]`)
                        .click()
                }
            })
        cy.get("div[data-cy='message-input']")
            .within(() => {
                cy.get("textarea")
                    .type(userMessagePairs[0][1])
                cy.get("button")
                    .click()
            })
    }

    function checkChatThreadContent(users: UserPool[], userMessagePairs: [UserPool, string][]) {
        ...
        const lastMessageAuthorVisibleName = userPoolToDetails[lastMessageAuthor].visibleName

        cy.get("div[data-cy='thread-previews-container']")
            .within(() => {
                cy.get("button").as("thread-preview")
                    .should("contain.text", `${threadName}${lastMessageAuthorVisibleName}: ${userMessagePairs[userMessagePairs.length - 1][1]}`)

                cy.get("@thread-preview")
                    .click()
            })

        cy.get("span[data-cy='thread-name']")
            .should("have.text", `${threadName}`)

        for (let i = 0; i < userMessagePairs.length; i++) {
            cy.get(`[data-cy="messages"] > :nth-child(${i + 1})`)
                .should("contain.text", userMessagePairs[i][1])
        }
    }

    function performSendingMessages(thisUser: UserPool, otherUsers: UserPool[], userMessagePairs: [UserPool, string][]) {
        cy.get("div[data-cy='thread-previews-container']")
            .within(() => {
                cy.get("button")
                    .click()
        ...
        cy.wait("@store-message").then((interception) => {
            ...
            cy.wrap(userMessagePairs).each(([user, message]: [UserPool, string]) => {
                if (user === thisUser) {
                    cy.get("div[data-cy='message-input']")
                        .within(() => {
                            cy.get("textarea")
