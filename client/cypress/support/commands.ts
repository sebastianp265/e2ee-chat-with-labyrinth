export const userPoolToDetails: Record<string, {
    visibleName: string,
...
Cypress.Commands.add('login', (username) => {
    ...
    cy.request({
        ...
        cy.window().then((win) => {
            ...
            win.localStorage.setItem('session_expires_at', (Date.now() + 60 * 60 * 1000).toString())
        })
    })
})
...
Cypress.Commands.add('connectToWebSocketAs', (username) => {
    cy.task("getSessionCookieOfUser", {username})
        .then(sessionCookie => {
