...
describe("labyrinth public key encryption", () => {
    test('sender encrypts the message and recipient decrypts it correctly', async () => {
        // recipient data
        const {publicKey: recipient_enc_pub, privateKey: recipient_enc_priv} = pk_enc_keygen()
        const psk = random(KEY_LENGTH_BYTES)
        // sender data
        const {publicKey: sender_auth_pub, privateKey: sender_auth_priv} = pk_auth_keygen()
        ...
        const decrypted_plaintext = await pk_decrypt(recipient_enc_pub, recipient_enc_priv, sender_auth_pub, psk, aad, ciphertext)
