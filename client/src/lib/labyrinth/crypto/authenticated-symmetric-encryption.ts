...
export function encrypt(key: Buffer, aad: Buffer, plaintext: Buffer) {
...
export function aes_gcm_256_encrypt(key: Buffer, nonce: Buffer, aad: Buffer, plaintext: Buffer) {
    const aes = Aes256GcmSiv.new(key)

    return Buffer.concat([
        nonce,
        aes.encrypt(plaintext, nonce, aad)
    ])
}
...
export function aes_gcm_256_decrypt(key: Buffer, aad: Buffer, ciphertext: Buffer) {
    ...
    return aes.decrypt(ciphertext.subarray(NONCE_LENGTH), nonce, aad)
}
