...
export async function labyrinth_hpke_encrypt(
    ...
    const { publicKey: pub_ephem, privateKey: priv_ephem } =
        generate_key_pair();
    ...
    console.log(`during encryption psk=${psk}`);
...
export async function labyrinth_hpke_decrypt(
    recipient_enc_pub: PublicKey,
    ...
    const fresh_secret = concat(id_id, id_ephem);
    console.log(`during decryption psk=${psk}`);
    const inner_aad = concat(
        Uint8Array.of(0x01),
        sender_auth_pub.getX25519PublicKeyBytes(),
        recipient_enc_pub.getX25519PublicKeyBytes(),
    ...
    const subkey = await kdf_one_key(fresh_secret, psk, inner_aad);
    const xd = await aes_gcm_256_decrypt(subkey, aad, ciphertext);
    return xd;
}
