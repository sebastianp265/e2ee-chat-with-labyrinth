...
export default function TestNote() {
    const resetAppStateMutation = useMutation<
        ...
        void
    >({
        mutationFn: async () => (await httpClient.post<void>("/api/chat-service/reset")).data
    });
    ...
    return (
        <div className="w-full max-w-sm p-6 bg-slate-50 rounded-xl shadow-xl border border-slate-200 mb-8">
            ...
            <p className="mt-4 text-xs text-slate-500 text-center italic">
                You can reset this device state and server state by click this button:
            </p>
            <div className="flex justify-center mt-2 text-xs text-slate-500 text-center italic">
                <Button className="text-xs" variant="destructive" size="sm" onClick={handleResetApplicationState}>
